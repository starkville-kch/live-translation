...
class CloudflareTunnelManager:
    def __init__(self, port: int = 8080, enabled: bool = True) -> None:
        ...
        self._tunnel_url: Optional[str] = PUBLIC_URL.rsplit("/live", 1)[0]
    ...
    @property
    def public_attendee_url(self):
        return PUBLIC_URL if self.is_ready else None
    ...
    def _public_check(self) -> bool:
        try:
            ...
            if sys.platform == "win32":
                try:
                    result = subprocess.run(
                        ["curl.exe", "-4", "-L", "--max-time", "15", "-o", "NUL", "-s", "-w", "%{http_code}", PUBLIC_URL],
    ...
    def _set_status(self, status: str, ready: bool = False) -> None:
        ...
        if ready and not was_ready:
            server_log.info("Public attendee link recovered")
