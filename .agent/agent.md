...
- **Attendee Web Audio Cross-Device Support**: On mobile browsers (iOS Safari in particular), `new AudioContext({ sampleRate: 24000 })` throws `NotSupportedError`. The client falls back to default `AudioContext` (which natively resamples the 24kHz buffer on playback), unlocks audio via a silent buffer on user gesture, and auto-resumes on touch if suspended. Server-side `/audio-stream` WebSocket sends empty keepalive frames during silence to prevent the 30s timeout from closing the socket.
